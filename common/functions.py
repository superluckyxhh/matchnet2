...
def mutual_nearest_neighbor_match(
    ...
    matches = torch.stack([b_ids, i_ids, j_ids]).T  # Nx3
...
def grid_positions(h, w, device='cpu', matrix=False):
    ...
    if matrix:
        return torch.stack([columns, rows], dim=0)
    else:
        return torch.cat([columns.view(1, -1), rows.view(1, -1)], dim=0)
